...
int uh_cgi_auth_check(
    ...
    if(!waf)
    {
        ...
        strncpy(waf->ipaddr, remote_addr, sizeof(waf->ipaddr) - 1);
        waf_list_add(waf);
    }

    /*
     * filter path which needn't auth 
     */
    if(strstr(pi->name, "/unauth.htm") ||
        strstr(pi->name, ".js") ||
        strstr(pi->name, ".css") || 
		strstr(pi->name, ".jpg") || 
		strstr(pi->name, ".gif") || 
		strstr(pi->name, ".png") ||
		strstr(pi->name, ".pac")) /* for wpad */
    {
        uh_file_request(cl, req, pi);
        return 0;
    }

    if(strstr(pi->name, "/unauth.cgi") ||
        strstr(pi->name, "/recover.cgi") ||
        strstr(pi->name, "/securityquestions.cgi") ||
        strstr(pi->name, "/multi_login.html") ||
        strstr(pi->name, "/change_user.html") ||
        strstr(pi->name, "/goodbye.html") ||
        strstr(pi->name, "/soap/server_sa/") ||
        strstr(pi->name, "/currentsetting.htm"))
    {
        return 1;
    }

    if(waf_item_check(waf))
    {
        recovery = config_get("enable_password_recovery");
        ...
        uh_http_sendf(cl, NULL, "<html><head>\n"
                "<script>\n"
                "top.location.href=\"/unauth.htm?recovery=%s\";\n"
                "</script>\n"
                "</head>\n"
                "<body bgcolor=\"#ffffff\">\n"
                "</body>\n"
                "</html>\n", 
                ((recovery[0] != '\0') ? recovery : "0"));
...
#define LOGIN_ERROR     "/tmp/AUTH_login_error"
#define LOGIN_BROWSER   "/tmp/AUTH_login_browser"
...
void echo_no_need_auth()
{
    FILE *fp = NULL;    
    unsigned char no_need_auth[4] = {0x1, 0x0, 0x0, 0x0};

    fp = fopen(NO_NEEDAUTH, "wb+");
    if(!fp)
    {
        return;
    }

    fwrite((const void *)no_need_auth, sizeof(no_need_auth), 1, fp);
