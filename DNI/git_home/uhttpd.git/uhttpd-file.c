...
static int uh_file_if_deflate(struct client *cl, struct http_request *req)
{
	char *hdr = uh_file_header_lookup(req, "Accept-Encoding");

	if(hdr && strstr(hdr, "gzip") != NULL)
	{
        return 0;
	}
...
void uh_file_request(struct client *cl, struct http_request *req, struct path_info *pi)
{
    ...
    /* force to http 1.1 to http 1.0, without chunk encoding */
    req->version = 1.0;

	/* we have a file */
	if( (pi->stat.st_mode & S_IFREG) && ((fd = open(pi->phys, O_RDONLY)) > 0) )
	{
        if((uh_file_if_deflate(cl, req) == 0) && (uh_gzip_init(cl) == 0))        
        {
            req->respond_gzipped = 1;
        }
		...
		if( ok > 0 )
		{
            ...
            if(req->respond_gzipped)
                ensure_out(uh_http_send(cl, NULL, "Content-Encoding: gzip\r\n", -1));
			...
			if( req->method != UH_HTTP_MSG_HEAD )
			{
				...
				while(1)
                {
                    memset(buf, 0x0, UH_LIMIT_MSGHEAD);
                    
                    rlen = read(fd, buf, UH_LIMIT_MSGHEAD);                    
                    if(rlen > 0)
                    {
                        if(req->respond_gzipped)
					        ensure_out(uh_gzip_http_send(cl, req, buf, rlen, 1));
                        else
                    ...
                    {
                        if(req->respond_gzipped)
                            ensure_out(uh_gzip_http_send(cl, req, buf, rlen, 0));
                        break; //done.
                    }
                    else
                    {
